...
async function insertServices(params) {
    try {
        var insertSave = await models.Menu.create(params);
...
async function listServices(params) {
    try {
        ...
        let listQuery = await models.Menu.findAll({
        ...
        let countRecords = await models.Menu.count({});
        return {
            status: true,
...
async function updateServices(params) {
    try {
        const { MENU_ID, MENU_NAME, MENU_SLUG } = params;
        var updateOne = await models.Menu.upsert(
            { MENU_ID: MENU_ID, MENU_NAME: MENU_NAME, MENU_SLUG: MENU_SLUG, UPDATE_DATE: new Date(), ISACTIVE: true }
        );
        if (updateOne) {
            return {
                status: true,
...
async function deleteServices(params) {
    try {
        ...
        var deleteOne = await models.Menu.update(
            { ISDELETE: true },
